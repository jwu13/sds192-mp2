...
```
### Introduction 
The present project is based on the datasets provided by the [Federal Election Committee](https://classic.fec.gov/finance/disclosure/ftpdet.shtml#archive_link) that include data about the various candidates, committees, contributions from committees to candidates they support, and House Elections results in 2012. We were interested in how candidates’ party affiliation and the type of entity making the transactions affect the relationship between the amount of transaction spent (couldn't find the unit, so we assumed in dollars) to support the candidates and the general votes they received.
...
#### Analysis
* From the graphs, we found that the there are different relationships between the amount of transaction spent by different entities to support House candidates and the general votes candidates received. 
* Though the data are very scattered from each other, we did find the general votes candidates received has a moderately positive relationship with the transactions made by Candidates, Candidate Committees and Committees, weakly positive relationships with those made by Political Action Committees and Individuals, and weakly negative relationships with those made by Organizations and Party Organizations. 
* Therefore, the transactions made by Candidates, Candidates Committees, and Committees could be considered more effective than those made by the other 4 types of entity if we can rule out possible confounding variables.
* The functions we created can be applied to other elections that has different types of entity making transactions, to other eneity types, and to other party affiliations. 

#### General Conclusion
*  Interestingly, the weak positive relationship between the amount of transaction used to support candidates running for the 2012 House Election and the general votes candidates received did not differ much by candidates' party affiliations (only for Democratic and Republican party in this case), but was influenced by the type of entity making the transactions. 
